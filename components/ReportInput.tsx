...
export const ReportInput: React.FC<ReportInputProps> = ({ onProcess, isLoading, error }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 md:p-6 space-y-4">
      <h2 className="text-lg md:text-xl font-semibold text-gray-700 dark:text-gray-300">
        Upload or Paste Your Medical Report
      </h2>
      
      {error && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg text-sm" role="alert">{error}</div>}
      ...
      <div className="space-y-3">
        ...
        <textarea
          ...
          className="w-full h-48 md:h-64 px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white transition resize-none"
          disabled={isLoading}
        />
      </div>
      ...
      <div className="flex items-center justify-center py-2">
        ...
        <span className="px-3 text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider">Or</span>
        <div className="flex-grow border-t border-gray-300 dark:border-gray-600"></div>
      </div>
      
      <div>
        <label className="w-full flex flex-col items-center justify-center px-4 py-5 bg-gray-50 dark:bg-gray-700/50 text-teal-600 dark:text-teal-400 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 cursor-pointer hover:bg-teal-50 dark:hover:bg-gray-700 transition">
          <UploadIcon />
          <span className="mt-2 text-sm text-center">{fileName || 'Upload a .txt, image, or PDF file'}</span>
